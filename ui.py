...
class HudCanvas(QWidget):
    ...
    def _preferred_camera_backends(self) -> list[int]:
        """Return Windows-friendly camera backend candidates.

        Some webcams open faster on DirectShow, while some laptop cameras only
        produce frames reliably on MSMF. We try safe fallbacks once instead of
        leaving the user on a black camera panel.
        """
        if cv2 is None:
            return [0]
        ...
        if _OS == "Windows":
            ...
            if hasattr(cv2, "CAP_MSMF"):
                add(cv2.CAP_MSMF)
            add(getattr(cv2, "CAP_ANY", 0))
        elif _OS == "Darwin":
            if hasattr(cv2, "CAP_AVFOUNDATION"):
                add(cv2.CAP_AVFOUNDATION)
            add(getattr(cv2, "CAP_ANY", 0))
        else:
            add(getattr(cv2, "CAP_ANY", 0))
    ...
    def start_camera_mode(self, camera_index: int | None = None) -> bool:
        ...
        """
        # Camera view must take priority over Security Map.
        # Previous build could keep security_map_mode=True, so the V6 map paint hook
        # kept drawing the map even after CAMERA VISION came online.
        try:
        ...
        try:
            ...
            opened = False
            last_error = ""
            for backend in self._preferred_camera_backends():
                try:
                    ...
                    try:
                        ...
                        pass
                    last_error = f"backend {backend} opened no frame"
                except Exception as backend_exc:
                    last_error = str(backend_exc)

            if not opened or not self._camera or not self._camera.isOpened():
                self._camera_error = f"Kamera açılamadı: index {self._camera_index}" + (f" · {last_error}" if last_error else "")
                self._camera = None
                self.update()
                return False

            try:
                # Lower preview size keeps the HUD smooth on Windows and is enough
                # for Gemini/OpenAI snapshot analysis.
            ...
            # Do not block the UI for a long warm-up. If the first frame is slow,
            # the timer will populate it and the camera panel stays visible.
            self._prime_camera_snapshot(max_reads=3)
    ...
    def _store_camera_frame(self, frame, force_snapshot: bool = False) -> bool:
        ...
        try:
            ...
            if force_snapshot or (self._tick % 8 == 0) or self._camera_snapshot_bytes is None:
...
class MainWindow(QMainWindow):
    ...
    def start_camera_mode(self, camera_index: int | None = None) -> bool:
        if not self.camera_access_enabled():
            ...
            return False
        try:
            # Switch HUD state immediately so clicking CAMERA after MAP does not
            # wait for another repaint cycle or leave the map layer on top.
            if hasattr(self, "hud") and self.hud is not None:
                self.hud.security_map_mode = False
                self.hud._security_map_dragging = False
                self.hud.camera_mode = True
                self.hud.state = "CAMERA"
                self.hud.update()
        except Exception:
            pass
    ...
    def _start_camera_mode_now(self, camera_index=None):
        ...
        if ok:
            ...
            try:
                if not self.hud.camera_snapshot_ready():
                    self._log.append_log("SYS: Camera panel opened; waiting for first frame.")
            except Exception:
                pass
        else:
...
def _mpv6_hud_paint_event(self, event):
    # Camera must win over Security Map. When a user clicks CAMERA while the
    # map is open, the old V6 paint hook still saw security_map_mode=True and
    # kept painting the map even though CAMERA VISION was online.
