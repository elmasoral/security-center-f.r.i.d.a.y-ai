...
def _mp_map_draw_trace(self, p: QPainter, rect: QRectF, trace: dict, kind: str):
    path = _mpv81_make_trace_path(self, rect, trace, kind)
    if path is None:
        return
...
def _mp_map_draw_point(self, p: QPainter, rect: QRectF, item: dict, kind: str, index: int):
    ...
    pt = _mpv81_project_continuous(self, lat, lng, rect)
    if not _mp_map_on_screen(pt, rect, 62.0):
        return

    source = item.get("source") if isinstance(item.get("source"), dict) else item
    risk = str(source.get("risk") or item.get("risk") or "LOW")
    color = _mp_map_risk_color(risk, kind)
    if kind == "threat" and color == C.PRI:
        color = "#ff6b35"

    pulse = (math.sin((getattr(self, "_tick", 0) * 0.16) + index * 0.8) + 1.0) * 0.5
    core_r = 5.8 + pulse * 1.6

    p.save()
    p.setClipRect(rect.adjusted(1, 1, -1, -1))

    glow = QRadialGradient(pt, 34)
    glow.setColorAt(0.00, qcol(color, 150))
    glow.setColorAt(0.48, qcol(color, 54))
    glow.setColorAt(1.00, qcol(color, 0))
    p.setPen(Qt.PenStyle.NoPen)
    p.setBrush(QBrush(glow))
    p.drawEllipse(pt, 34, 34)

    p.setBrush(QBrush(qcol(color, 235)))
    p.drawEllipse(pt, core_r, core_r)
    p.setBrush(QBrush(qcol("#ffffff", 235)))
    p.drawEllipse(pt, max(2.2, core_r * 0.45), max(2.2, core_r * 0.45))

    p.setPen(QPen(qcol(color, 220), 2.0))
    p.setBrush(Qt.BrushStyle.NoBrush)
    p.drawEllipse(pt, core_r + 6.0, core_r + 6.0)
    p.setPen(QPen(qcol(color, 85), 1.0))
    p.drawEllipse(pt, core_r + 15.0 + pulse * 5.0, core_r + 15.0 + pulse * 5.0)

    if float(getattr(self, "_security_map_zoom", 1.0) or 1.0) > 2.2 or index < 12:
        label = str(source.get("city") or source.get("label") or source.get("country") or source.get("ip") or "")[:24]
        if label:
            p.setFont(QFont("Segoe UI", 7, QFont.Weight.Black))
            p.setPen(qcol("#f4fbff", 220))
            p.drawText(QRectF(pt.x() + 12, pt.y() - 11, 150, 22), Qt.AlignmentFlag.AlignLeft | Qt.AlignmentFlag.AlignVCenter, label)
...
def _mp_map_draw_target(self, p: QPainter, rect: QRectF):
    ...
    label = str(target.get("url") or target.get("label") or "Protected website")
    pulse = (math.sin(getattr(self, "_tick", 0) * 0.115) + 1.0) * 0.5
